...
function tournament_selection(population::Vector{Individual}, instance::ProblemInstance, fitness_cache::Dict{UInt64, Individual}, locker::ReentrantLock, k::Int=3, penalty_cost::Float64=10.0)::Tuple{Int, Int}
    idx_candidates1 = rand(1:length(population), k)
    candidates1 = [population[i] for i in idx_candidates1]
    evaluated_candidates1 = [evaluate(x, instance, fitness_cache, locker, 1.0, max(div(penalty_cost, 10), 1.0), penalty_cost, penalty_cost) for x in candidates1]
    ...
    evaluated_candidates2 = [evaluate(x, instance, fitness_cache, locker, 1.0, max(div(penalty_cost, 10), 1.0), penalty_cost, penalty_cost) for x in candidates2]
